...
def run_analysis(model, val_loader, device, config):
    """
    Run all analysis functions after training.
    """
    print("\n" + "="*50)
    ...
    # 1. Visualize latent hierarchy
    print("\n1. Visualizing latent hierarchy...")
    latent_data = visualize_latent_hierarchy(model, val_loader, device)
    
    # 2. Measure disentanglement
    print("\n2. Measuring disentanglement...")
    disentanglement_results = measure_disentanglement(model, val_loader, device)
    
    # 3. Analyze latent dimensions
    print("\n3. Analyzing latent dimensions...")
    analyze_latent_dimensions(model, val_loader, device)
    
    # 4. Generate controllable samples
    print("\n4. Generating controllable samples...")
    generated_patterns = controllable_generation(model, None, device)
    
    # 5. Style interpolation
    print("\n5. Creating style interpolations...")
    # Get two patterns from different styles for interpolation
    pattern1 = None
    pattern2 = None
    
    with torch.no_grad():
        for patterns, styles, densities in val_loader:
            # Find patterns from different styles
            rock_indices = (styles == 0).nonzero(as_tuple=True)[0]
            jazz_indices = (styles == 1).nonzero(as_tuple=True)[0]
            
            if len(rock_indices) > 0 and len(jazz_indices) > 0:
                pattern1 = patterns[rock_indices[0]]  # rock pattern
                pattern2 = patterns[jazz_indices[0]]  # jazz pattern
                break
    
    if pattern1 is not None and pattern2 is not None:
        interpolations = interpolate_styles(model, pattern1, pattern2, device=device)
    else:
        print("Warning: Could not find patterns from different styles for interpolation")
        interpolations = None
...
def main():
    ...
    history_for_json = history.copy()
    history_for_json['config'] = config.copy()
    history_for_json['config']['device'] = str(config['device'])
    ...
    run_analysis(model, val_loader, config['device'], config)
